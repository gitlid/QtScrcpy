...
public static class Updater {
    public const string Version = "0.4.3-rc.1";
    public const string PayloadHash = "ed4a5b091485b9d3a8f7871e983e1d0a2dfdb44141866fd46edea82230278d04";
    const string Prefix = "QtScrcpy-keymapper-0.4.3-rc.1-win64/";
    ...
    public static void ValidateInstallation(string target, string updaterLocation) {
        ...
        if (Environment.OSVersion.Platform == PlatformID.Win32NT) {
            ...
            if (v.FileMajorPart != 0 || v.FileMinorPart != 4 || v.FileBuildPart > 3)
                throw new IOException("仅允许更新 0.4.0—0.4.3 系列，拒绝降级或覆盖无法识别的程序。检测版本：" + v.FileVersion);
            ...
            if (File.Exists(info)) {
                ...
                if (match.Success && match.Groups[1].Value.StartsWith("0.4.3") && match.Groups[1].Value!=Version)
                    throw new IOException("检测到不同或更新的 0.4.3 版本，拒绝把它降级为候选版："+match.Groups[1].Value);
            }
        }
        Idle();
    }
