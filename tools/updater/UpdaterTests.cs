...
class UpdaterTests {
    ...
    static MemoryStream Zip(IDictionary<string,string> entries) {
        ...
        using(var z=new ZipArchive(s,ZipArchiveMode.Create,true)) foreach(var x in entries) using(var w=new StreamWriter(z.CreateEntry("QtScrcpy-keymapper-0.4.5-rc.1-win64/"+x.Key).Open(),Encoding.UTF8)) w.Write(x.Value);
