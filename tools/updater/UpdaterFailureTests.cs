...
class FailureTests {
    ...
    static MemoryStream Package(){var s=new MemoryStream();using(var z=new ZipArchive(s,ZipArchiveMode.Create,true)){foreach(var x in new[]{"QtScrcpy.exe","Qt5Core.dll","a.dll","platforms/qwindows.dll","build-info.json"})using(var w=new StreamWriter(z.CreateEntry("QtScrcpy-keymapper-0.4.3-rc.1-win64/"+x).Open()))w.Write("new-"+x);}s.Position=0;return s;}
